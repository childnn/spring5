...
class KotlinNoSuchBeanFailureAnalyzerNoKotlinReflectTests {
	...
	fun failureAnalysisForConfigurationPropertiesThatMaybeShouldHaveBeenConstructorBound() {
		...
		assertThat(analysis!!.action).startsWith(
			java.lang.String.format(
				"Consider defining a bean of type '%s' in your configuration.",
				String::class.java.getName()
			)
		)
		assertThat(analysis.getAction()).contains(
			java.lang.String.format(
				"Consider adding a dependency on kotlin-reflect so that the constructor used for @ConstructorBinding can be located. Also, ensure that @ConstructorBinding is present on '%s' ",
				ConstructorBoundProperties::class.java.getName()
			)
		)
	}
