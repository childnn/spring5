...
class KotlinConstructorParametersBinderTests {
	...
	fun `Bind to class when has no prefix should create bound bean`() {
		...
		val binder = Binder(source)
		val bean = binder.bind(
			ConfigurationPropertyName.of(""),
			Bindable.of(ExampleValueBean::class.java)
		).get()
		assertThat(bean.intValue).isEqualTo(12)
		assertThat(bean.longValue).isEqualTo(34)
		assertThat(bean.booleanValue).isTrue()
	...
	fun `Bind to data class should create bound bean`() {
		...
		val expectedBean = ExampleDataClassBean(
			intValue = 12,
			longValue = 34,
			booleanValue = true,
			stringValue = "foo",
	...
	fun `Bind to class should bind nested`() {
		val source = MockConfigurationPropertySource(
			mapOf(
				"foo.value-bean.int-value" to "123",
	...
	fun `Bind to class with no value for primitive should use default value`() {
		...
		val bean = binder.bind(
			"foo", Bindable.of(
				ExamplePrimitiveDefaultBean::class.java
			)
		).get()
	...
	fun `Bind to data class with generics`() {
		...
		val bean = binder.bind(
			"foo", Bindable
				.of<GenericValue<Map<String, String>>>(
					ResolvableType.forClassWithGenerics(
						GenericValue::class.java,
						type
					)
				)
		)
	...
	class ExampleValueBean(
		val intValue: Int?, val longValue: Long?,
		val booleanValue: Boolean?, val stringValue: String?,
		val enumValue: ExampleEnum?
	)

	class ExamplePrimitiveDefaultBean(
		val intValue: Int = 0, val longValue: Long = 0,
	...
	class ExampleDefaultValueBean(
		val intValue: Int = 5,
		val stringsList: List<String> = listOf("a", "b", "c"),
		val customList: List<String> = listOf("x,y,z")
	)

	data class ExampleDataClassBean(
		val intValue: Int = 5, val longValue: Long = 42,
